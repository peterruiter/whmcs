...
$gatewaymodule = "molliepaysafecard"; # Enter your gateway module name here
...
if (isset($transactionId)) {
    ...
    if ($paysafecard->getPaidStatus() == true) {
        ...
        logTransaction($GATEWAY["name"], $_POST, "Unsuccessful - Status: " . $paysafecard->getBankStatus); # Save to Gateway Log: name, data array, status
        exit;
    }
}
...
if (isset($transactionId)) {
    addInvoicePayment($orderId, $transactionId, $amount, $fee, $gatewaymodule); # Apply Payment to Invoice: invoiceid, transactionid, amount paid, fees, modulename
    logTransaction($GATEWAY["name"], $_POST, "Successful - Status: " . $paysafecard->getBankStatus); # Save to Gateway Log: name, data array, status
}
